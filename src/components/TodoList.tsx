  ...
  return (
    <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar pb-20">
      <div className="flex items-center justify-between mb-5 sticky top-0 bg-background-main z-10 py-2">
        <h3 className="text-xl font-bold text-primary-charcoal">資金タスク</h3>
      </div>
      <button onClick={onAddTodo} className="w-full py-4 rounded-xl border-2 border-dashed border-cool-blue/30 text-cool-blue font-semibold hover:bg-cool-blue/5 hover:border-cool-blue transition-all flex items-center justify-center gap-2 group mb-6">
      ...
      <div className="flex border-b border-border-light mb-6">
        <button
          ...
          className={`flex-1 py-3 text-center font-semibold transition-colors ${
            activeTab === 'incomplete' ? 'text-cool-blue border-b-2 border-cool-blue' : 'text-dark-blue-gray hover:text-primary-charcoal'
          }`}
        >
          未完了 ({transferTodos.filter(t => !t.completed).length})
        </button>
        <button
          onClick={() => setActiveTab('completed')}
          className={`flex-1 py-3 text-center font-semibold transition-colors ${
            activeTab === 'completed' ? 'text-cool-blue border-b-2 border-cool-blue' : 'text-dark-blue-gray hover:text-primary-charcoal'
          }`}
        >
          完了 ({completedCount})
        </button>
